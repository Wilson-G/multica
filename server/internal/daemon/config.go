...
type Config struct {
	...
	Agents             map[string]AgentEntry // "claude" -> entry, "codex" -> entry, "droid" -> entry, "opencode" -> entry, "openclaw" -> entry, "hermes" -> entry
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codex, droid, opencode, openclaw, or hermes and ensure it is on PATH")
	}
