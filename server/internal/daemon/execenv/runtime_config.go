...
// For Droid:    writes {workDir}/AGENTS.md  (skills discovered natively via DROID_HOME/CODEX_HOME-compatible layout)
...
func InjectRuntimeConfig(workDir, provider string, ctx TaskContextForEnv) error {
	...
	case "codex", "droid", "opencode", "openclaw":
...
func buildMetaSkillContent(provider string, ctx TaskContextForEnv) string {
	...
	if len(ctx.AgentSkills) > 0 {
		...
		case "codex", "droid", "opencode", "openclaw":
			// Codex-compatible and other native providers discover skills from their respective paths — just list names.
			b.WriteString("You have the following skills installed (discovered automatically):\n\n")
		default:
