...
// handleTaskActivity records an activity for task terminal events.
func handleTaskActivity(ctx context.Context, bus *events.Bus, queries *db.Queries, e events.Event, action string) {
	...
	terminalState, _ := payload["terminal_state"].(string)
	terminalReason, _ := payload["terminal_reason"].(string)
	message, _ := payload["message"].(string)
	supersededByTaskID, _ := payload["superseded_by_task_id"].(string)
	supersededByAgentID, _ := payload["superseded_by_agent_id"].(string)
	...
	if action == "task_failed" && terminalState == "blocked" {
		action = "task_blocked"
	}
	...
	details, err := json.Marshal(map[string]string{
		...
		"superseded_by_agent_id": supersededByAgentID,
	})
	if err != nil {
		slog.Error("activity: failed to marshal task activity details",
			"issue_id", issueID, "action", action, "error", err)
		return
	}
